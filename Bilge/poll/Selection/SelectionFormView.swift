                ...
                GButton(labelText: "Save",
                    ...
                    withAnimation(.easeInOut(duration: 0.4)) {
                        if question.isEmpty || options.filter({ $0.isEmpty }).count > 0 || options.isEmpty{
                            ...
                            let component = Selection(question: question,
                                                      time: selection,
                                                      options: options)
                            
                            let counters = pollManager.counters
                            var count = counters[PollComponentType.selection.toString()] ?? 0
                            count+=1
                            
                            pollManager.counters[PollComponentType.selection.toString()] = count
...
struct SelectionFormView_Previews: PreviewProvider {
    static var previews: some View {
        NavigationView {
            SelectionFormView().navigationTitle("algo")
        }
    }
}
